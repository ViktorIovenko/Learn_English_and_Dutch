# bot/auth.py
# ParallelLingvo Telegram bot registration + persistent keyboard
# [ИЗМЕНЕНО v6.0] Убран ?uid= из Mini App URL и персональный fallback-пароль.
...
def _get_expected_password() -> str:
    # Public ParallelLingvo can run without a shared registration password.
    # If BOT_PASSWORD/REG_PASSWORD is configured, the legacy password gate remains available.
...
def _build_app_url(user_id: int) -> tuple[str, bool, bool]:
    # Identity is verified by Telegram initData inside the Mini App.
    # Never put a user id in a browser-controlled URL.
    base = f"{Config.PUBLIC_BASE_URL}".rstrip("/")
    url = base + "/"
    if _is_https(base): return (url, True, True)
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not expected:
        # Public registration mode: Telegram itself already identifies the user.
        _register_user(Config.DB_PATH, user)
        await show_menu_with_keyboard(update, context, user.id)
...
async def send_open(update: Update, context: ContextTypes.DEFAULT_TYPE, hello: str = "") -> None:
    ...
    if use_webapp:
        kb_inline = InlineKeyboardMarkup([[InlineKeyboardButton(text="Открыть ParallelLingvo", web_app=WebAppInfo(url=url))]])
        m = await update.effective_chat.send_message(hello or "Откройте ParallelLingvo:", reply_markup=kb_inline)
        asyncio.create_task(_delete_later(context, m.chat_id, m.message_id))
    elif use_inline:
        kb_inline = InlineKeyboardMarkup([[InlineKeyboardButton(text="Открыть ParallelLingvo", url=url)]])
        m = await update.effective_chat.send_message(hello or f"Откройте ParallelLingvo:\n{url}", reply_markup=kb_inline)
        asyncio.create_task(_delete_later(context, m.chat_id, m.message_id))
    else:
        await _ephemeral_send(update, context, hello or f"Откройте ParallelLingvo:\n{url}")
    await show_menu_with_keyboard(update, context, user_id)
